...
func testAccPrometheusOperatorPrometheusExists(n string, obj *po_types.Prometheus) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		out, err := conn.Prometheuses(namespace).Get(context.TODO(), name, meta_v1.GetOptions{})
		if err != nil {
			return err
		}
...
func testAccPrometheusOperatorPrometheusDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		resp, err := conn.Prometheuses(namespace).Get(context.TODO(), name, meta_v1.GetOptions{})
		if err == nil {
